...
export const FinancialReportMockup: React.FC<{ theme?: string; disableHover?: boolean; initialScaleDown?: boolean; noAspectRatio?: boolean }> = ({
  ...
  // Shared utilities
  const cols = "grid grid-cols-[minmax(90px,1.5fr)_repeat(5,minmax(55px,1fr))]";
  ...
  const HighlightRow: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <div className={`${cols} rounded-md bg-violet-50/60 text-slate-900 font-semibold py-0.5`}>
      {children}
    </div>
  );

  const Row: React.FC<{ muted?: boolean; children: React.ReactNode }> = ({ muted, children }) => (
    <div className={`${cols} ${muted ? "text-slate-600" : "text-slate-800"} py-0.5`}>{children}</div>
  );
...
export const KpiDashboardMockup: React.FC<{ theme?: string, noAspectRatio?: boolean, disableHover?: boolean, initialScaleDown?: boolean, className?: string }> = ({ noAspectRatio = false, disableHover = false, initialScaleDown = false, className }) => {
  ...
  return (
    <div className={classNames.filter(Boolean).join(' ')}>
      <div className="w-full text-left mb-2 sm:mb-4 anim-child" style={{'--i': 0} as React.CSSProperties}>
        <h3 className="font-bold text-gray-900 text-xs sm:text-sm">Tableau de Bord - T4 2024</h3>
      </div>
      <div className="flex-grow grid grid-cols-2 grid-rows-2 gap-2 sm:gap-3">
        ...
        <div className="bg-gray-50 rounded-lg p-2 sm:p-3 border border-gray-100 col-span-1 row-span-1 anim-child" style={{'--i': 2} as React.CSSProperties}>
          <p className="text-xs text-gray-700">Taux de Churn</p>
          <p className="font-bold text-gray-800 text-base sm:text-lg md:text-xl">2.1% <span className="text-green-500 text-sm">↓</span></p>
        </div>
         <div className="bg-gray-50 rounded-lg p-2 sm:p-3 border border-gray-100 col-span-1 row-span-1 anim-child" style={{'--i': 3} as React.CSSProperties}>
          <p className="text-xs text-gray-700">CAC</p>
          <p className="font-bold text-[#27013D] text-base sm:text-lg md:text-xl">1.2k€ <span className="text-red-500 text-sm">↑</span></p>
        </div>
      </div>
    </div>
  );
...
export const services = [
    ...
    { 
        slug: 'services-ma', 
        title: 'Services M&A', 
        subTitle: 'Accompagner', 
        description: 'vos projets capitalistiques : cession, levée de fonds, croissance externe et réorganisation.', 
        icon: <img src="https://images2.imgbox.com/fb/c3/An8Qt7JF_o.png" alt="Services M&A Icon" />,
        darkIcon: <img src="https://images2.imgbox.com/91/54/9teZ4nY8_o.png" alt="Services M&A Icon" />,
